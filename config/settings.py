...
# ──────────────────────────────────────────────────────────────
# L5 多智能体编排：出站通知营销 Agent（旁路，可关闭）
# ──────────────────────────────────────────────────────────────
# 留空 = 关闭。关闭时退货申请等事件不会外发，ZT 单机运行零依赖。
# 开启后：用户提交退货申请 → POST {MKT_AGENT_BASE_URL}/api/operations/coordination/inbound
#         → 营销侧登记一条协调事件，供运营复盘该商品售后策略。
# 端口提示：营销 Agent 后端默认 8010（8001 是 ZT-agent 自己在 Docker 下的宿主机端口）。
# 发送在后台守护线程内进行（超时 3s、异常只记日志），绝不影响订单状态机。
MKT_AGENT_BASE_URL = os.getenv('MKT_AGENT_BASE_URL', '')
# 与营销侧 COORD_SHARED_SECRET 一致；未配置时营销侧开发模式放行
COORD_SHARED_SECRET = os.getenv('COORD_SHARED_SECRET', '')
